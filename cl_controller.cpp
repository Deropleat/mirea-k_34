...
cl_controller::cl_controller() : cl_base(nullptr, "")
{
}

cl_controller::cl_controller(cl_base* p_head_object) : cl_base(p_head_object, "cl_controller")
{
	p_operator = new cl_operator(this);
}

void cl_controller::update()
{
	for (size_t i = 0; i < gas_stations.size(); i++) gas_stations.at(i)->update();
}

void cl_controller::gasoline_handler(std::string& message)
{
...
void cl_controller::commands_handler(std::string& message)
{
	...
	else if (message.find("Display the petrol filling station status") != std::string::npos)
	{
		...
		message = "Petrol filling stations status " + number + " " + std::to_string(gas_stations.at(index)->get_in_queue()) + " " + std::to_string(gas_stations.at(index)->get_is_ordered()) + gas_stations.at(index)->get_ordered_list();

	}
	else if (message == "Display the system status")
	{
		message = p_operator->get_gas_stations_status(gas_stations);
	}
	else if (message.empty()) message.clear();
}